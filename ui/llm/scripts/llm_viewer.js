export class LlmViewer {
    ...
    async showContextPopup() {
        ...
        popup.querySelector('.llm-close-btn').onclick = () => popup.remove();
    ...
    async sendMessage() {
        if (!this.currentChatId) return alert('Создайте чат');
        const text = this.inputEl.value.trim();
        if (!text) return;
        this.addMessage(text, 'user');
        this.inputEl.value = '';
        const thinking = document.createElement('div');
        thinking.className = 'llm-message assistant-msg';
        thinking.textContent = 'Думаю...';
        this.pageEl.appendChild(thinking);

        try {
            const res = await fetch(`${this.api}/llm/chats/${this.currentChatId}/search?request=${encodeURIComponent(text)}`);
            const data = await res.json();
            thinking.remove();
            this.addMessage(data.response || 'Нет ответа', 'assistant', data.documents || []);
        } catch (e) {
            thinking.textContent = 'Ошибка';
        }
    }
